	...
	public function getAttri() {
        ...
        */
        public function setKey(){
            ...
            $codeKey = UdpControl::generateControl(UCOT_IA, UCSOT_IA_CODE, UCAT_CODE_RC_KEY);
            $codeKey['type'] = 2;
            $codeKey['key'] = 128;//0：新增  自定义按键id：修改
            $codeKey['irid'] = 715;
            $codeKey['devid'] = 2;
            $codeKey['flags'] = 1;
            $codeKey['name'] = 'a';
        ...
        */
        public function postLearn(){
            ...
            * 
            */
            $unpackData = $this->_udpCtrl->excuteControl($devSn, $codeLearn, UCA_PUSH);
...
//$wkController->setMatch();//云匹配请求匹配
//$wkController->getMatch();//云匹配获取匹配状态
...
//$wkController->deleteLearn();//取消遥控器按键学习请求----自测通过
$wkController->putLearn();//遥控器按键学习请求（UCA_SET）----自测通过
//$wkController->postLearn();//遥控器按键学习状态（UCA_PUSH）
