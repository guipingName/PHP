...
class Error {
    ...
    public static function getError($code,$errcode = 'code',$errmsg = 'message') {
        $error = Error::$GLOBAL_ERROR;
        if (isset($code) && isset($error[$code])) {
            return array($errcode => $code, $errmsg => $error[$code]);
        } else {
            return array($errcode => E_SYSTEM_UNKNOWN, $errmsg => $error[E_SYSTEM_UNKNOWN]);
        }
    }
}