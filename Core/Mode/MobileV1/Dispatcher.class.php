...
class Dispatcher {
    ...
    static public function dispatch($path_info,$dispatch) {
        ...
        if ($path_info_url && C('MULTI_MODULE')){ // 获取模块名
            $paths  =  explode($depr,$path_info_url,3);
            $path_info_url = isset($paths[2]) ? $paths[2]: '';
            $module = ucfirst(preg_replace('/\.' . $path_info_ext . '$/i', '',$paths[1])); 
        } else {
            return $dispatch;
        }
        $allowList =   C('MODULE_ALLOW_LIST'); // 允许的模块列表
        $denyList  =   C('MODULE_DENY_LIST'); // 禁止的模块列表
        /*检查模块是否存在以及访问权限*/
        ...
        if(count($paths) == 0){
            ...
            /*
             * 访问格式：
             /api/v1/lock/{page}/{pageSize}
             /api/v1/lock/{deviceUuid}/passwords
             /api/v1/lock/{page}/{pageSize}
             */
            $controller = $paths[0];
            $action = $paths[1];
            ...
            /*
             * 访问格式：
             /api/v1/lock/{deviceUuid}/passwords/{passwordId}
             */
            $controller = $paths[0];
            $action = $paths[1];
            ...
            /*
             * 访问格式：
             /api/v1/lock/{deviceUuid}/events/{page}/{pageSize}
             */
            $controller = $paths[0];
            $action = $paths[1];
        }
